...
const { compressMessages } = require('./compression');
const adapters = {
...
// A hung upstream connection would otherwise stall a request forever —
// this bounds how long any single provider attempt gets before Bifrost
// gives up on it and falls through to the next account/step, same as a
// hard error. Configurable via settings; a very long value effectively
// disables it.
const DEFAULT_REQUEST_TIMEOUT_MS = 120_000;
function requestTimeoutMs() {
  return getSetting('request_timeout_ms', DEFAULT_REQUEST_TIMEOUT_MS);
}
...
async function attemptChain(comboId, overrideBody) {
  ...
  // Compress old tool-result content before it goes anywhere — applied
  // once here, so it benefits both /v1/chat/completions and /v1/messages
  // (which both funnel through this same OpenAI-shape body).
  if (overrideBody?.messages) {
    const compressionConfig = getSetting('compression', {});
    const { messages: compressed } = compressMessages(overrideBody.messages, compressionConfig);
    overrideBody = { ...overrideBody, messages: compressed };
  }
  ...
  for (const step of orderSteps(combo)) {
    ...
    for (const account of eligibleAccounts(provider.id)) {
      ...
      try {
        ...
        const message = err.name === 'AbortError' ? `timed out after ${requestTimeoutMs() / 1000}s` : err.message;
        errors.push(`${provider.name}/${account.label}: ${message}${cb.tripped ? ' — circuit breaker OPEN' : ''}`);
      }
    }
  }
...
async function attemptEmbeddings(comboId, { input }) {
  const combo = getCombo(comboId);
  if (!combo) throw new Error('No combo found (create one, or mark one as default, in the dashboard)');
  ...
  for (const step of orderSteps(combo)) {
    ...
    for (const account of eligibleAccounts(provider.id)) {
      const timeout = withTimeout();
      try {
        const res = await adapter.embeddings({ apiKey: account.api_key, baseUrl: provider.base_url, model: step.model, input, signal: timeout.signal });
        timeout.clear();
        if (!res.ok) {
          const cb = recordFailure(account.id, 15);
          const text = await res.text().catch(() => '');
        ...
        // model in the response reflects what the route actually used, same
        // convention as /v1/chat/completions — not whatever the client asked for.
        const result = adapter.normalizeEmbeddingsResponse(json, step.model);
        return { result, provider, account, model: step.model, comboId: combo.id };
      } catch (err) {
        timeout.clear();
        const cb = recordFailure(account.id, 15);
        const message = err.name === 'AbortError' ? `timed out after ${requestTimeoutMs() / 1000}s` : err.message;
...
async function testAccount({ provider, account, model }) {
  ...
  try {
    ...
    timeout.clear();
    return { ok: false, error: err.name === 'AbortError' ? `timed out after ${requestTimeoutMs() / 1000}s` : err.message };
  }
}
