  ...
  for (const step of combo.steps) {
    ...
    for (const account of eligibleAccounts(provider.id)) {
      ...
      try {
        ...
        if (res.status === 429) {
          // Rate limited: cool this account down and try the next one/provider.
          markCooldown(account.id, 60);
          errors.push(`${provider.name}/${account.label}: rate limited (429)`);
          continue;
        }
        if (!res.ok) {
...
module.exports = { attemptChain, estimateCost, logRequest, getCombo, DEFAULT_PRICING };
